...
- **Google Gemini** (via the official `@google/genai` SDK, model `gemini-2.5-flash`) for summarization, Bengali rewriting, and categorization
...
- Each individual article's AI generation is cached *separately*, keyed by a
  ...
  translation instead of paying for a fresh Gemini call every cycle.
...
lib/
  ai.ts                 All Gemini API calls (summarize, rewrite, categorize)
...
3. Add `GEMINI_API_KEY` and `NEXT_PUBLIC_SITE_URL` as environment
...
2. **Check `GEMINI_API_KEY`** is set in your deployment environment —
